...
class phpMyEdit_export extends phpMyEdit
{
    ...
    public function list_table()
    {
        ...
        for ($k = 0; $k < $this->num_fds; $k++) {
            ...
            $cells .= $this->substituteVars($tmpl, ['value' => $value, 'name' => $value]);
        }
        ...
        $export_header = $this->substituteVars($row_tmpl, ['cells' => $cells]);
        ...
        while ($row = mysqli_fetch_array($res, MYSQLI_ASSOC)) {
            ...
            for ($k = 0; $k < $this->num_fds; $k++) {
                ...
                if (is_array($func)) {
                    foreach ($func as $fnc) {
                        if (is_callable($fnc)) {
                            $value = $fnc((string) $value);
                        }
                    }
                } elseif (is_callable($func)) {
                    $value = $func((string) $value);
                }
                $cells .= $this->substituteVars($tmpl, ['value' => $value]);
            }
            ...
            foreach ($row as $key => $val) {
                if (preg_match('/^qf(\d+)$/', $key, $matches)) {
                    $idx = $matches[1];
                    if (isset($this->fds[$idx])) {
                        $assoc[$this->fds[$idx]] = $val;
                    }
                }
            }
        ...
        echo $this->substituteVars($content_tmpl, ['header' => $export_header, 'rows' => $export_rows]);
        exit;
    }
